...
15. chat.ts (store)
    - 路径: packages/client/src/stores/hermes/chat.ts
    - 说明: 远程模式发送 source='api_server'，Socket.IO 路由到 handleApiRun（走代理到远端 Agent）

16. handle-api-run.ts (服务端)
    - 路径: packages/server/src/services/hermes/run-chat/handle-api-run.ts
    - 说明: resolveRunSource 尊重 client source；upstream/apiKey 从环境变量读取

17. TerminalPanel.vue
...
Write-Host "   - packages/client/src/stores/hermes/chat.ts"
Write-Host "   - packages/client/src/shared/session-display.ts"
...
Write-Host "   - packages/server/src/services/hermes/run-chat/handle-api-run.ts"
