...
function buildWsUrl(): string {
  ...
  // 开发模式直连 Koa（terminal WebSocket 由 Koa 提供，远程 Agent 无此端点）
  if (import.meta.env.DEV) {
    return `${wsProtocol}//${formatHostForPort(location.hostname, 8648)}/api/hermes/terminal${token ? `?token=${encodeURIComponent(token)}` : ""}`;
  }

  const base = getBaseUrlValue();
  if (base) {
    const proto = base.startsWith("https") ? "wss:" : "ws:";
    return `${proto}//${new URL(base).host}/api/hermes/terminal${token ? `?token=${encodeURIComponent(token)}` : ""}`;
  }
