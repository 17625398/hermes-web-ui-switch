...
/**
 * Check Hermes gateway status — returns parsed output
 */
export async function gatewayStatus(): Promise<{ running: boolean; pid?: number; detail: string }> {
  try {
    const { stdout, stderr } = await execFileAsync(HERMES_BIN, ['gateway', 'status'], {
      timeout: 10000,
      ...activeGatewayExecOpts(),
    })
    const detail = (stdout || stderr || '').trim()
    const running = detail.toLowerCase().includes('running')
    const pidMatch = detail.match(/PID:\s*(\d+)/i)
    return { running, pid: pidMatch ? Number(pidMatch[1]) : undefined, detail }
  } catch {
    return { running: false, detail: 'Failed to check gateway status' }
  }
}
